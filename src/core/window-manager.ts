...
import { cloneWindowOptions, createWindowOptions, type WindowOptions, type WindowOptionsOverride } from './window-options'
...
export type WindowManagerChangeKind = 'open' | 'focus' | 'close' | 'geometry' | 'layout' | 'pane' | 'options' | 'snap' | 'maximize' | 'minimize' | 'restore' | 'lock' | 'unlock'
...
function cloneWindow(window: WindowState): WindowState { return { ...window, rootPane: clonePaneTree(window.rootPane), options: cloneWindowOptions(window.options), snap: window.snap ? { zone: window.snap.zone, floatingGeometry: cloneGeometry(window.snap.floatingGeometry) } : null, restoreGeometry: window.restoreGeometry ? cloneGeometry(window.restoreGeometry) : null, ...(window.layoutSpec !== undefined ? { layoutSpec: window.layoutSpec ? cloneWindowLayoutSpec(window.layoutSpec) : null } : {}), geometry: cloneGeometry(window.geometry), constraints: { minSize: cloneSize(window.constraints.minSize), maxSize: window.constraints.maxSize ? cloneSize(window.constraints.maxSize) : null } } }
...
function resolveActiveWindowLayouts(windows: readonly WindowState[], container: WindowSize, sourceInstanceIds?: readonly WindowInstanceId[]): WindowState[] {
  validateWindowLayoutReferences(windows)
...
export class WindowManager {
  ...
  private responsiveContainer: WindowSize | null = null
  ...
  getResponsiveContainer(): WindowSize | null { return this.responsiveContainer ? { ...this.responsiveContainer } : null }
  ...
  setOptions(instanceId: WindowInstanceId, override: WindowOptionsOverride, origin: WindowOperationOrigin = 'api'): WindowState { const current = this.windows.find((window) => window.instanceId === instanceId); if (!current) throw new UnknownWindowInstanceError(instanceId); const legacyOpacityStyle = current.options.surfaceStyle && override.surfaceStyle === undefined && override.opacity !== undefined ? { ...current.options.surfaceStyle, opacity: override.opacity } : undefined; const updated: WindowState = { ...current, options: createWindowOptions({ ...current.options, ...override, ...(legacyOpacityStyle ? { surfaceStyle: legacyOpacityStyle } : {}) }) }, replaced = this.windows.map((window) => window.instanceId === instanceId ? updated : window); this.windows = updated.focused ? focusWithinLayer(replaced, updated) : stackWindows(replaced); this.emit('options', origin, instanceId); return this.get(instanceId) }
  ...
  setGeometry(instanceId: WindowInstanceId, geometry: WindowGeometry, origin: WindowOperationOrigin = 'api'): WindowState {
    ...
    this.emit(materialized ? 'layout' : 'geometry', origin, instanceId)
  ...
  setLayoutSpec(instanceId: WindowInstanceId, layoutSpec: WindowLayoutSpec | null, container?: WindowSize, origin: WindowOperationOrigin = 'user', layoutSpecState?: WindowLayoutRuleState): WindowState {
    ...
    this.emit('layout', origin, instanceId)
    return this.get(instanceId)
  }

  resolveResponsiveLayouts(container: WindowSize, origin: WindowOperationOrigin = 'api'): readonly WindowState[] {
    validateWindowLayoutReferences(this.windows)
    this.responsiveContainer = { ...container }
    const updatedWindows = resolveActiveWindowLayouts(this.windows, container)
    let changed = false
    let firstChangedId: string | undefined
    for (const [index, window] of updatedWindows.entries()) {
      const previous = this.windows[index]
      if (!previous || sameGeometry(previous.geometry, window.geometry)) continue
      changed = true
      firstChangedId ??= window.instanceId
    }
    if (!changed) return this.list()
    this.windows = updatedWindows
    this.emit('geometry', origin, firstChangedId ?? updatedWindows[0]?.instanceId ?? '')
    return this.list()
  }
  ...
  constrainToContainer(instanceId: WindowInstanceId, container: WindowSize, origin: WindowOperationOrigin = 'api'): WindowState {
    ...
    if (current.layoutLocked && current.layoutSpec) {
      this.resolveResponsiveLayouts(container, origin)
      return this.get(instanceId)
    }
  ...
  private openPaneState(request: { pane: PaneNode; instanceId: WindowInstanceId; title: string; titleIsCustom: boolean; options: WindowOptions; snap: WindowSnapState | null; restoreGeometry: WindowGeometry | null; layoutLocked: boolean; layoutSpec?: WindowLayoutSpec | null; layoutSpecState?: WindowLayoutRuleState; position?: WindowPosition | undefined; size?: WindowSize | undefined; minSize?: WindowSize | undefined; maxSize?: WindowSize | undefined; defaultSize?: WindowSize | undefined }, origin: WindowOperationOrigin): WindowState {
    ...
    const opened: WindowState = { instanceId: request.instanceId, title: request.title, titleIsCustom: request.titleIsCustom, rootPane: clonePaneTree(request.pane), options: cloneWindowOptions(request.options), snap: request.snap ? { zone: request.snap.zone, floatingGeometry: cloneGeometry(request.snap.floatingGeometry) } : null, restoreGeometry: request.restoreGeometry ? cloneGeometry(request.restoreGeometry) : null, layoutLocked: request.layoutLocked, ...(request.layoutSpec !== undefined ? { layoutSpec: request.layoutSpec ? cloneWindowLayoutSpec(request.layoutSpec) : null } : {}), ...(request.layoutSpecState ? { layoutSpecState: request.layoutSpecState } : request.layoutSpec ? { layoutSpecState: request.layoutLocked ? 'active' as const : 'dormant' as const } : request.layoutLocked ? { layoutSpecState: 'materialized' as const } : {}), focused: true, zIndex: 0, mode: 'normal', geometry: { position: { x: Number.isFinite(position.x) ? position.x : cascade, y: Number.isFinite(position.y) ? position.y : cascade }, size }, constraints }
