...
export type WindowManagerChangeKind = 'open' | 'focus' | 'close' | 'geometry' | 'pane' | 'options' | 'snap' | 'maximize' | 'minimize' | 'restore'
...
export interface OpenPaneWindowRequest { pane: PaneNode; instanceId?: WindowInstanceId; title?: string; position?: WindowPosition; size?: WindowSize; minSize?: WindowSize; maxSize?: WindowSize; options?: WindowOptionsOverride; snap?: WindowSnapState | null; restoreGeometry?: WindowGeometry | null }
...
function stackWindows(windows: readonly WindowState[], focusedId?: WindowInstanceId): WindowState[] { const normal = windows.filter((window) => window.options.layer === 'normal'); const alwaysOnTop = windows.filter((window) => window.options.layer === 'always-on-top'); return [...normal, ...alwaysOnTop].map((window, zIndex) => ({ ...window, focused: focusedId === undefined ? window.focused : window.instanceId === focusedId, zIndex })) }
function focusWithinLayer(windows: readonly WindowState[], target: WindowState): WindowState[] { const remaining = windows.filter((window) => window.instanceId !== target.instanceId); if (target.options.layer === 'normal') { const normal = remaining.filter((window) => window.options.layer === 'normal'), alwaysOnTop = remaining.filter((window) => window.options.layer === 'always-on-top'); return stackWindows([...normal, target, ...alwaysOnTop], target.instanceId) } return stackWindows([...remaining, target], target.instanceId) }
...
function normalizePane(registry: WidgetRegistry, pane: PaneNode): PaneNode { validatePaneTree(pane); if (pane.kind === 'widget') { const resolved = registry.resolve(pane.widgetId, pane.parameters); return { ...clonePaneTree(pane), parameters: paneParameters(resolved.parameters) } } return { ...clonePaneTree(pane), children: pane.children.map((child) => normalizePane(registry, child)) } }
function defaultPaneTitle(registry: WidgetRegistry, pane: PaneNode): string { return pane.kind === 'widget' ? registry.get(pane.widgetId).title : 'Workspace' }
...
export class WindowManager {
  ...
  open(request: OpenWindowRequest, origin: WindowOperationOrigin = 'api'): WindowState {
    ...
    return this.openPaneState({ pane, instanceId, title: request.title ?? resolved.manifest.title, position: request.position, size: request.size, minSize: manifestWindow?.minSize, maxSize: manifestWindow?.maxSize, defaultSize: manifestWindow?.defaultSize, options: createWindowOptions({ ...(manifestWindow?.options ?? {}), ...(request.options ?? {}) }), snap: null, restoreGeometry: null }, origin)
  }
  openPane(request: OpenPaneWindowRequest, origin: WindowOperationOrigin = 'api'): WindowState { const instanceId = request.instanceId ?? this.createInstanceId(), pane = normalizePane(this.registry, request.pane); return this.openPaneState({ pane, instanceId, title: request.title ?? defaultPaneTitle(this.registry, pane), position: request.position, size: request.size, minSize: request.minSize, maxSize: request.maxSize, options: createWindowOptions(request.options), snap: request.snap ?? null, restoreGeometry: request.restoreGeometry ?? null }, origin) }
  focus(instanceId: WindowInstanceId, origin: WindowOperationOrigin = 'api'): WindowState { const current = this.windows.find((window) => window.instanceId === instanceId); if (!current) throw new UnknownWindowInstanceError(instanceId); if (current.mode === 'minimized') return this.restore(instanceId, origin); const sameLayer = this.windows.filter((window) => window.options.layer === current.options.layer); if (sameLayer.at(-1)?.instanceId === instanceId && current.focused) { this.setActiveLifecycle(instanceId); return cloneWindow(current) } this.windows = focusWithinLayer(this.windows, current); this.setActiveLifecycle(instanceId); this.emit('focus', origin, instanceId); return this.get(instanceId) }
  ...
  private openPaneState(request: { pane: PaneNode; instanceId: WindowInstanceId; title: string; options: WindowOptions; snap: WindowSnapState | null; restoreGeometry: WindowGeometry | null; position?: WindowPosition | undefined; size?: WindowSize | undefined; minSize?: WindowSize | undefined; maxSize?: WindowSize | undefined; defaultSize?: WindowSize | undefined }, origin: WindowOperationOrigin): WindowState {
    ...
    const opened: WindowState = { instanceId: request.instanceId, title: request.title, rootPane: clonePaneTree(request.pane), options: { ...request.options }, snap: request.snap ? { zone: request.snap.zone, floatingGeometry: cloneGeometry(request.snap.floatingGeometry) } : null, restoreGeometry: request.restoreGeometry ? cloneGeometry(request.restoreGeometry) : null, focused: true, zIndex: 0, mode: 'normal', geometry: { position: { x: Number.isFinite(position.x) ? position.x : cascade, y: Number.isFinite(position.y) ? position.y : cascade }, size }, constraints }
  ...
  private createLifecycle(instanceId: WindowInstanceId): WidgetLifecycleController { const lifecycle = createWidgetLifecycle(instanceId); let unsubscribe: () => void = () => {}; unsubscribe = lifecycle.subscribe((event) => { if (event.kind !== 'destroy') return; this.lifecycles.delete(instanceId); unsubscribe() }); this.lifecycles.set(instanceId, lifecycle); return lifecycle }
