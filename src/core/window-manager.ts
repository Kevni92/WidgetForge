...
export interface OpenPaneWindowRequest { pane: PaneNode; instanceId?: WindowInstanceId; title?: string; /** Used by workspace restore to retain auto-title semantics for older snapshots. */ titleIsCustom?: boolean; position?: WindowPosition; size?: WindowSize; minSize?: WindowSize; maxSize?: WindowSize; options?: WindowOptionsOverride; snap?: WindowSnapState | null; restoreGeometry?: WindowGeometry | null }
export interface OpenLauncherWindowRequest { instanceId?: WindowInstanceId; title?: string; position?: WindowPosition; size?: WindowSize; minSize?: WindowSize; maxSize?: WindowSize; options?: WindowOptionsOverride; snap?: WindowSnapState | null; restoreGeometry?: WindowGeometry | null }
...
export class WindowManager {
  ...
  openPane(request: OpenPaneWindowRequest, origin: WindowOperationOrigin = 'api'): WindowState { const instanceId = request.instanceId ?? this.createInstanceId(), pane = normalizePane(this.registry, request.pane); return this.openPaneState({ pane, instanceId, title: request.title ?? defaultPaneTitle(this.registry, pane), titleIsCustom: request.titleIsCustom ?? request.title !== undefined, position: request.position, size: request.size, minSize: request.minSize, maxSize: request.maxSize, options: createWindowOptions(request.options), snap: request.snap ?? null, restoreGeometry: request.restoreGeometry ?? null }, origin) }
  openEmptyWindow(request: OpenLauncherWindowRequest = {}, origin: WindowOperationOrigin = 'api'): WindowState {
    ...
    return this.openPaneState({ pane, instanceId, title: request.title ?? DEFAULT_LAUNCHER_WINDOW_TITLE, titleIsCustom: request.title !== undefined, position: request.position, size: request.size, defaultSize: DEFAULT_LAUNCHER_WINDOW_SIZE, minSize: request.minSize, maxSize: request.maxSize, options: createWindowOptions(request.options), snap: request.snap ?? null, restoreGeometry: request.restoreGeometry ?? null }, origin)
  }
  ...
  private openPaneState(request: { pane: PaneNode; instanceId: WindowInstanceId; title: string; titleIsCustom: boolean; options: WindowOptions; snap: WindowSnapState | null; restoreGeometry: WindowGeometry | null; position?: WindowPosition | undefined; size?: WindowSize | undefined; minSize?: WindowSize | undefined; maxSize?: WindowSize | undefined; defaultSize?: WindowSize | undefined }, origin: WindowOperationOrigin): WindowState {
    ...
    const opened: WindowState = { instanceId: request.instanceId, title: request.title, titleIsCustom: request.titleIsCustom, rootPane: clonePaneTree(request.pane), options: { ...request.options }, snap: request.snap ? { zone: request.snap.zone, floatingGeometry: cloneGeometry(request.snap.floatingGeometry) } : null, restoreGeometry: request.restoreGeometry ? cloneGeometry(request.restoreGeometry) : null, focused: true, zIndex: 0, mode: 'normal', geometry: { position: { x: Number.isFinite(position.x) ? position.x : cascade, y: Number.isFinite(position.y) ? position.y : cascade }, size }, constraints }
