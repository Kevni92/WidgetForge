...
export class WindowGroupDefinitionError extends Error { constructor(message:string){super(message);this.name='WindowGroupDefinitionError'} }
const managerGroups = new WeakMap<WindowManager, WindowGroupManager>()
...
export class WindowGroupManager {
  ...
  constructor(private readonly windows:WindowManager){
    const existing=managerGroups.get(windows);if(existing)existing.dispose()
    managerGroups.set(windows,this)
    this.unsubscribeWindowManager=windows.subscribe((change)=>{if(change.kind==='close'||change.kind==='snap'||change.kind==='maximize')this.remove(change.instanceId)})
  }
  list():readonly WindowGroupState[]{return[...this.groups.entries()].map(([id,members])=>({id,members:[...members]}))}
  get(groupId:WindowGroupId):WindowGroupState|undefined{const members=this.groups.get(groupId);return members?{id:groupId,members:[...members]}:undefined}
  groupOf(instanceId:WindowInstanceId):WindowGroupId|null{for(const[groupId,members]of this.groups)if(members.includes(instanceId))return groupId;return null}
  assign(groupId:WindowGroupId,instanceIds:readonly WindowInstanceId[]):WindowGroupState{
    const id=groupId.trim();if(!id)throw new WindowGroupDefinitionError('window group id must not be empty')
    const members=[...new Set(instanceIds)];if(members.length===0)throw new WindowGroupDefinitionError('window group must contain at least one member')
    for(const instanceId of members){const window=this.windows.get(instanceId);if(window.mode!=='normal'||window.snap)throw new WindowGroupDefinitionError(`window "${instanceId}" must be floating before it can be grouped`);this.remove(instanceId,false)}
    this.groups.set(id,members);this.emit();return{id,members:[...members]}
  }
  ...
  restoreGroup(groupId:WindowGroupId,origin:WindowOperationOrigin='user'):void{for(const instanceId of this.groups.get(groupId)??[])if(this.windows.get(instanceId).mode==='minimized')this.windows.restore(instanceId,origin)}
  closeGroup(groupId:WindowGroupId,origin:WindowOperationOrigin='user'):void{const members=[...(this.groups.get(groupId)??[])];for(const instanceId of members)this.windows.close(instanceId,origin)}
  subscribe(listener:WindowGroupListener):()=>void{this.listeners.add(listener);return()=>this.listeners.delete(listener)}
  dispose():void{this.unsubscribeWindowManager();this.listeners.clear();this.groups.clear();if(managerGroups.get(this.windows)===this)managerGroups.delete(this.windows)}
  private emit():void{const change:WindowGroupChange={groups:this.list()};for(const listener of[...this.listeners])listener(change)}
}
export function createWindowGroupManager(windows:WindowManager):WindowGroupManager{return new WindowGroupManager(windows)}
export function getWindowGroupManager(windows:WindowManager):WindowGroupManager|undefined{return managerGroups.get(windows)}
