...
export interface WorkspaceSnapshot { readonly version:typeof WORKSPACE_VERSION; readonly windows:readonly WorkspaceWindowSnapshot[]; readonly docks:readonly WorkspaceDockSnapshot[] }
...
export type WorkspaceRestoreIssueCode = 'invalid-workspace'|'unsupported-version'|'manager-not-empty'|'dock-manager-not-empty'|'dock-manager-required'|'invalid-window'|'invalid-dock'|'duplicate-instance'|'duplicate-dock'|'unknown-widget'|'invalid-parameters'|'singleton-conflict'|'open-failed'|'dock-open-failed'
export interface WorkspaceRestoreIssue { readonly code:WorkspaceRestoreIssueCode; readonly message:string; readonly index?:number; readonly instanceId?:string; readonly widgetId?:WidgetId; readonly dockId?:string }
export interface WorkspaceRestoreResult { readonly valid:boolean; readonly restored:readonly WindowState[]; readonly restoredDocks:readonly DockState[]; readonly issues:readonly WorkspaceRestoreIssue[] }
...
export function serializeWorkspace(manager:WindowManager,dockManager?:DockManager):string{return JSON.stringify(captureWorkspace(manager,dockManager))}
function invalidRoot(code:WorkspaceRestoreIssueCode,message:string):WorkspaceRestoreResult{return{valid:false,restored:[],restoredDocks:[],issues:[{code,message}]}}
...
function readParameters(value:unknown):Record<string,WorkspaceParameterValue>|null{if(!isRecord(value))return null;const result:Record<string,WorkspaceParameterValue>={};for(const[key,candidate]of Object.entries(value)){if(typeof candidate!=='string'&&typeof candidate!=='number'&&typeof candidate!=='boolean')return null;if(typeof candidate==='number'&&!Number.isFinite(candidate))return null;result[key]=candidate}return result}
function readGeometry(value:unknown):WindowGeometry|null{if(!isRecord(value)||!isRecord(value.position)||!isRecord(value.size))return null;const{x,y}=value.position;const{width,height}=value.size;if(!isFiniteNumber(x)||!isFiniteNumber(y)||!isFiniteNumber(width)||!isFiniteNumber(height)||width<=0||height<=0)return null;return{position:{x,y},size:{width,height}}}
...
function readDock(value:unknown):WorkspaceDockSnapshot|null{
  ...
  const rootPane=readPane(value.rootPane);if(!rootPane||!isFiniteNumber(value.thickness)||value.thickness<0||!isFiniteNumber(value.minThickness)||value.minThickness<0)return null
  const max=value.maxThickness===null?null:value.maxThickness;if(max!==null&&(!isFiniteNumber(max)||max<value.minThickness))return null
...
export function restoreWorkspace(manager:WindowManager,input:unknown,dockManager?:DockManager):WorkspaceRestoreResult{
  ...
  if(dockManager){dockValues.forEach((value,index)=>{const dock=readDock(value);if(!dock){issues.push({code:'invalid-dock',message:'invalid workspace dock entry',index});return}try{dockManager.add({id:dock.id,position:dock.position,pane:dock.rootPane,thickness:dock.thickness,minThickness:dock.minThickness,...(dock.maxThickness!==null?{maxThickness:dock.maxThickness}:{}),resizable:dock.resizable})}catch(error){issues.push({code:error instanceof DuplicateDockError?'duplicate-dock':'dock-open-failed',message:error instanceof Error?error.message:'dock restore failed',index,dockId:dock.id})}})}
