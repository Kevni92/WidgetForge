...
export interface WorkspaceEditSnapshot {
  ...
  readonly paneLocks: readonly WorkspacePaneSelection[]
}
...
export class WorkspaceEditController {
  ...
  get state(): WorkspaceEditState {
    return {
      ...
      paneLocks: cloneLocks(this.paneLocks),
    }
  }
  ...
  setMode(mode: WorkspaceEditMode): WorkspaceEditState {
    ...
    this.mode = mode; this.temporaryEdit = false
  ...
  selectPane(selection: WorkspacePaneSelection | null): WorkspaceEditState {
    ...
    this.selection = next; this.emit(); return this.state
  }

  setPaneLocked(selection: WorkspacePaneSelection, locked: boolean): WorkspaceEditState {
    const paneKey = key(selection)
    const exists = this.paneLocks.some((candidate) => key(candidate) === paneKey)
    if (exists === locked) return this.state
    this.paneLocks = locked
      ? [...this.paneLocks, cloneSelection(selection) as WorkspacePaneSelection]
      : this.paneLocks.filter((candidate) => key(candidate) !== paneKey)
    this.emit(); return this.state
  }
  ...
  restore(snapshot: WorkspaceEditSnapshot): WorkspaceEditState {
    ...
    this.mode = snapshot.mode; this.temporaryEdit = false
    this.selection = snapshot.mode === 'edit' ? cloneSelection(snapshot.selection) : null
    this.paneLocks = cloneLocks(snapshot.paneLocks)
    this.emit(); return this.state
  }
...
export function retargetWidgetPane(root: PaneNode, paneId: string, widgetId: WidgetId, parameters: PaneParameters = {}): PaneNode {
  ...
  return replacePane(root, paneId, createWidgetPane({ id: pane.id, widgetId, instanceId: pane.instanceId, parameters, ...(pane.settings ? { settings: pane.settings } : {}) }))
}

export function removePaneForEdit(root: PaneNode, paneId: string): PaneNode | null { return removePane(root, paneId).root }

export function createPaneEditContextMenuItems(root: PaneNode, paneId: string, locked: boolean): readonly ContextMenuItem[] {
  const pane = findPane(root, paneId)
  if (!pane) return []
  if (locked) return [{ id: 'unlock', label: 'Unlock pane' }]
  const items: ContextMenuItem[] = [{ id: 'split', label: 'Split pane…' }]
