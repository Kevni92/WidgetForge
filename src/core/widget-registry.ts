...
export class DuplicateWidgetIdError extends Error {
  constructor(public readonly widgetId: WidgetId) {
...
export class WidgetParameterValidationError extends Error {
  constructor(
    ...
    public readonly issues: WidgetParameterIssue[],
  ) {
    super(`invalid parameters for widget "${widgetId}"`)
    this.name = 'WidgetParameterValidationError'
  }
}
...
export function validateWidgetParameters(
  manifest: WidgetManifest,
  input: Readonly<Record<string, unknown>> = {},
): WidgetParameterValidationResult {
  const schema = manifest.parameters ?? {}
  const issues: WidgetParameterIssue[] = []
  const parameters: Record<string, unknown> = {}

  for (const parameter of Object.keys(input)) {
  ...
  for (const [parameter, definition] of Object.entries(schema)) {
    ...
    if (!supplied) {
      if (definition.default !== undefined) {
        parameters[parameter] = definition.default
    ...
    if (!matchesParameterType(value, definition)) {
      issues.push({
        parameter,
        code: 'type',
        message: `parameter "${parameter}" must be a ${definition.type}`,
      })
      continue
    }
...
export class WidgetRegistry {
  ...
  constructor(manifests: readonly WidgetManifest[] = []) {
    for (const manifest of manifests) this.register(manifest)
  }

  register(manifest: WidgetManifest): void {
    const validatedManifest = defineWidget(manifest)
    if (this.manifests.has(validatedManifest.id)) throw new DuplicateWidgetIdError(validatedManifest.id)
    this.manifests.set(validatedManifest.id, validatedManifest)
  }
  ...
  has(widgetId: WidgetId): boolean {
    return this.manifests.has(widgetId)
  }

  get(widgetId: WidgetId): WidgetManifest {
    const manifest = this.manifests.get(widgetId)
    if (!manifest) throw new UnknownWidgetError(widgetId)
    return manifest
  }
  ...
  validate(widgetId: WidgetId, parameters: Readonly<Record<string, unknown>> = {}): WidgetParameterValidationResult {
    return validateWidgetParameters(this.get(widgetId), parameters)
  }

  resolve(widgetId: WidgetId, parameters: Readonly<Record<string, unknown>> = {}): ResolvedWidget {
    const manifest = this.get(widgetId)
    const result = validateWidgetParameters(manifest, parameters)
...
export function createWidgetRegistry(manifests: readonly WidgetManifest[] = []): WidgetRegistry {
  return new WidgetRegistry(manifests)
}
