...
const unsubscribe=manager.subscribe((change)=>{
  ...
  if((change.kind==='open'||change.kind==='restore')&&containerSize.value.width>0&&containerSize.value.height>0&&!manager.get(change.instanceId).layoutSpec)manager.constrainToContainer(change.instanceId,containerSize.value)
...
onMounted(()=>{if(!hostElement.value)return;disposeSizeObserver=observeElementSize(hostElement.value,(size)=>{containerSize.value=size;if(snapPreview.value)snapPreview.value={...snapPreview.value,geometry:snapWindowGeometry(snapPreview.value.zone,size)};manager.resolveResponsiveLayouts(size,'api');for(const window of manager.list())if(!window.layoutSpec)manager.constrainToContainer(window.instanceId,size)});ensureModalFocus();scheduleModalFocus();hostElement.value.addEventListener('focusin',onFocusIn,true);globalThis.document.addEventListener('focusin',onFocusIn,true);globalThis.window.addEventListener('keydown',onGlobalKeyDown)})
