...
import { detectWindowSnapZone, snapWindowGeometry, windowSnapZoneLabel, type WindowSnapZone } from '../core/window-snap'
...
const hostElement=ref<HTMLElement|null>(null),containerSize=shallowRef<WindowSize>({width:0,height:0}),windows=shallowRef<readonly WindowState[]>(manager.list()),groupStates=shallowRef<readonly WindowGroupState[]>(groups?.list()??[]),snapPreview=shallowRef<SnapPreviewState|null>(null),windowDockPreview=shallowRef<WindowDockPreviewState|null>(null),resizePreviewIds=shallowRef<readonly string[]>([])
...
function windowIsLayoutLocked(instanceId:string):boolean{return manager.list().find((window)=>window.instanceId===instanceId)?.layoutLocked??false}
...
const unsubscribe=manager.subscribe((change)=>{
  ...
  if((change.kind==='open'||change.kind==='restore')&&containerSize.value.width>0&&containerSize.value.height>0){const state=manager.get(change.instanceId);if(!state.layoutLocked||!state.layoutSpec)manager.constrainToContainer(change.instanceId,containerSize.value)}
...
function previewSnap(instanceId:string,zone:WindowSnapZone|null):void{if(props.layoutLocked||interactionBlocked(instanceId)||windowIsLayoutLocked(instanceId)){snapPreview.value=null;return}if(!zone){if(snapPreview.value?.instanceId===instanceId)snapPreview.value=null;return}snapPreview.value={instanceId,zone,geometry:snapWindowGeometry(zone,containerSize.value)}}
function commitSnap(instanceId:string,zone:WindowSnapZone):void{if(props.layoutLocked||interactionBlocked(instanceId)||manager.get(instanceId).layoutLocked)return;snapPreview.value=null;manager.snapWindow(instanceId,zone,containerSize.value,'user')}
function unsnapForPointer(instanceId:string,clientX:number,clientY:number):WindowState{const current=manager.get(instanceId);if(current.layoutLocked)return current;const point=localPoint(clientX,clientY);return point?manager.unsnapWindow(instanceId,point,containerSize.value,'user'):manager.unsnapWindow(instanceId,undefined,undefined,'user')}
...
function targetWindowElement(sourceInstanceId:string,x:number,y:number):HTMLElement|null{const host=hostElement.value;if(!host||props.layoutLocked||interactionBlocked(sourceInstanceId)||manager.get(sourceInstanceId).layoutLocked)return null;return[...host.querySelectorAll<HTMLElement>('.wf-window-frame[data-window-instance-id]')].filter((element)=>element.dataset.windowInstanceId!==sourceInstanceId&&element.dataset.windowMode==='normal'&&element.dataset.windowLayoutLocked!=='true'&&element.getAttribute('aria-hidden')!=='true').filter((element)=>containsClientPoint(element.getBoundingClientRect(),x,y)).sort((a,b)=>Number(b.dataset.windowZIndex??0)-Number(a.dataset.windowZIndex??0))[0]??null}
...
function resolveWindowDrop(sourceInstanceId:string,x:number,y:number):WindowDropTarget|null{if(props.layoutLocked||groupId(sourceInstanceId)||interactionBlocked(sourceInstanceId))return null;const source=manager.get(sourceInstanceId);if(source.layoutLocked||source.options.role==='modal'||source.options.role==='overlay')return null;const windowElement=targetWindowElement(sourceInstanceId,x,y);if(!windowElement)return null;const paneElement=targetPaneElement(windowElement,x,y),targetInstanceId=windowElement.dataset.windowInstanceId,targetPaneId=paneElement?.dataset.paneId;if(!paneElement||!targetInstanceId||!targetPaneId||manager.get(targetInstanceId).layoutLocked)return null;const rect=paneElement.getBoundingClientRect(),zone=detectWorkspaceDropZone({x,y},{x:rect.left,y:rect.top,width:rect.width,height:rect.height});return zone?{targetInstanceId,targetPaneId,zone}:null}
function previewWindowDrop(sourceInstanceId:string,target:WindowDropTarget|null):void{if(props.layoutLocked||interactionBlocked(sourceInstanceId)||windowIsLayoutLocked(sourceInstanceId)){windowDockPreview.value=null;return}if(!target){if(windowDockPreview.value?.sourceInstanceId===sourceInstanceId)windowDockPreview.value=null;return}const host=hostElement.value,targetWindow=findWindowElement(target.targetInstanceId),paneElement=targetWindow?findPaneElement(targetWindow,target.targetPaneId):null;if(!host||!paneElement||windowIsLayoutLocked(target.targetInstanceId))return;const hostRect=host.getBoundingClientRect(),paneRect=paneElement.getBoundingClientRect();windowDockPreview.value={sourceInstanceId,target,targetRect:{x:paneRect.left-hostRect.left,y:paneRect.top-hostRect.top,width:paneRect.width,height:paneRect.height}}}
function commitWindowDrop(sourceInstanceId:string,target:WindowDropTarget):void{if(props.layoutLocked||groupId(sourceInstanceId)||interactionBlocked(sourceInstanceId)||manager.get(sourceInstanceId).layoutLocked||manager.get(target.targetInstanceId).layoutLocked)return;windowDockPreview.value=null;windowDockSequence+=1;try{dockWindowIntoWindow(manager,sourceInstanceId,target.targetInstanceId,target.targetPaneId,target.zone,`window-dock-${windowDockSequence}`)}catch{windowDockPreview.value=null}}
...
onMounted(()=>{if(!hostElement.value)return;disposeSizeObserver=observeElementSize(hostElement.value,(size)=>{const before=manager.list();containerSize.value=size;if(snapPreview.value)snapPreview.value={...snapPreview.value,geometry:snapWindowGeometry(snapPreview.value.zone,size)};manager.resolveResponsiveLayouts(size,'api');for(const window of manager.list())if(!window.layoutLocked||!window.layoutSpec)manager.constrainToContainer(window.instanceId,size);const previous=new Map(before.map((window)=>[window.instanceId,window]));resizePreviewIds.value=manager.list().filter((window)=>window.layoutLocked&&window.layoutSpec&&previous.get(window.instanceId)&&JSON.stringify(previous.get(window.instanceId)?.geometry)!==JSON.stringify(window.geometry)).map((window)=>window.instanceId)});ensureModalFocus();scheduleModalFocus();hostElement.value.addEventListener('focusin',onFocusIn,true);globalThis.document.addEventListener('focusin',onFocusIn,true);globalThis.window.addEventListener('keydown',onGlobalKeyDown)})
onBeforeUnmount(()=>{snapPreview.value=null;windowDockPreview.value=null;resizePreviewIds.value=[];disposeSizeObserver?.();disposeSizeObserver=null;unsubscribe();unsubscribeGroups();hostElement.value?.removeEventListener('focusin',onFocusIn,true);globalThis.document.removeEventListener('focusin',onFocusIn,true);globalThis.window.removeEventListener('keydown',onGlobalKeyDown);modalRestoreTargets.clear();modalLastFocus.clear();pendingModalRestore=null})
</script>
<template><div ref="hostElement" class="wf-window-manager-host" tabindex="-1" data-window-focus-fallback :data-layout-locked="layoutLocked||undefined" :data-modal-active="topModal()?.instanceId||undefined"><div v-if="topModal()" class="wf-window-manager-host__modal-backdrop" data-modal-backdrop aria-hidden="true"/><div v-if="snapPreview&&!layoutLocked" class="wf-window-snap-preview" :data-window-snap-preview="snapPreview.instanceId" :data-window-snap-zone="snapPreview.zone" :style="geometryStyle(snapPreview.geometry)" role="status"><strong>{{ windowSnapZoneLabel(snapPreview.zone) }}</strong><span>Release to snap as layout</span></div><DockingOverlay v-if="windowDockPreview&&!layoutLocked" :target-rect="windowDockPreview.targetRect" :active-zone="windowDockPreview.target.zone" :source-id="windowDockPreview.sourceInstanceId" :target-id="windowDockPreview.target.targetInstanceId"/><WindowFrame v-for="window in windows" :key="window.instanceId" :window="window" :manager="manager" :registry="registry" :container-size="containerSize" :command-registry="props.commands" :launcher-navigator="navigator" :launcher-placeholder="props.launcherPlaceholder" :launcher-submit-label="props.launcherSubmitLabel" :lifecycle="manager.getLifecycle(window.instanceId)" :layout-locked="layoutLocked" :edit-mode="editMode" :resize-preview="resizePreviewIds.includes(window.instanceId)" :pane-drag-enabled="(paneId) => paneDragEnabledForWindow(window.instanceId, paneId)" :interaction-blocked="interactionBlocked(window.instanceId)" :groups="groups" :group-id="groupId(window.instanceId)" :resolve-snap-zone="resolveSnapZone" :preview-snap="previewSnap" :commit-snap="commitSnap" :unsnap-for-pointer="unsnapForPointer" :resolve-window-drop="resolveWindowDrop" :preview-window-drop="previewWindowDrop" :commit-window-drop="commitWindowDrop"/></div></template>
<style scoped>.wf-window-manager-host{position:relative;width:100%;height:100%;min-width:0;min-height:0;overflow:hidden}.wf-window-manager-host__modal-backdrop{position:absolute;inset:0;z-index:calc(var(--wf-layer-window) + 2999);background:var(--wf-color-backdrop);pointer-events:auto}.wf-window-snap-preview{position:absolute;z-index:var(--wf-layer-overlay);display:grid;place-content:center;gap:var(--wf-space-xs);pointer-events:none;border:1px solid var(--wf-color-focus);border-radius:var(--wf-radius-sm);background:var(--wf-color-selected);box-shadow:inset 0 0 0 1px var(--wf-color-border);color:var(--wf-color-text);text-align:center}.wf-window-snap-preview span{font-size:var(--wf-font-size-xs);color:var(--wf-color-text-muted)}</style>
