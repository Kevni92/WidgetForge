<style scoped>
.wf-pane-host .wf-pane-host__drag-glyph {
  display: inline-grid;
  width: var(--wf-size-icon-size);
  height: var(--wf-size-icon-size);
  place-items: center;
  font-size: var(--wf-size-icon-size);
  line-height: 1;
}
.wf-pane-host .wf-pane-host__tabbar {
  min-height: var(--wf-size-tab-height);
}
.wf-pane-host .wf-pane-host__tab {
  min-height: var(--wf-size-tab-height);
  padding-inline: var(--wf-space-sm);
}
.wf-pane-host .wf-pane-host__tab-drag-handle {
  font-size: var(--wf-size-icon-size);
}
</style>
...
const visible = computed(
  () => props.active && props.hostVisible && !collapsed.value,
);
...
const actionMaxVisible = computed(() =>
  measuredSize.value.width > 0 && measuredSize.value.width < 220
    ? 1
    : measuredSize.value.width > 0 && measuredSize.value.width < 340
      ? 2
      : 3,
);
const compactActions = computed(
  () => measuredSize.value.width > 0 && measuredSize.value.width < 300,
);
function setWidgetActions(bindings: readonly WidgetActionBinding[]): void {
  widgetActions.value = bindings;
  emit("actionsChange", bindings);
}
...
function childStyle(split: SplitPane, index: number): Record<string, string> {
  const child = split.children[index],
    weight = split.weights[index] ?? 1;
  const style: Record<string, string> = { minWidth: "0", minHeight: "0" };
  if (!child) return style;
  if (child.settings?.collapsed) {
    style.display = "none";
    return style;
  }
  const mode = child.settings?.sizeMode ?? "flex",
    min = child.settings?.minSize,
    max = child.settings?.maxSize;
  if (mode === "fixed") {
    ...
    if (split.axis === "horizontal") style.width = "max-content";
    else style.height = "max-content";
  } else {
    style.flexGrow = String(weight * (child.settings?.grow ?? 1));
    style.flexShrink = "1";
    style.flexBasis = "0";
  }
  if (split.axis === "horizontal") {
    if (min !== undefined) style.minWidth = `${min}px`;
    if (max !== undefined) style.maxWidth = `${max}px`;
  } else {
    if (min !== undefined) style.minHeight = `${min}px`;
    if (max !== undefined) style.maxHeight = `${max}px`;
  }
  return style;
}
function stackChildStyle(
  index: number,
  pane: PaneNode,
): Record<string, string> {
  return {
    zIndex: String(index),
    ...(pane.settings?.collapsed ? { display: "none" } : {}),
  };
}
function dividerResizable(split: SplitPane, index: number): boolean {
  if (
    props.layoutLocked ||
    split.settings?.resizable === false ||
    split.settings?.locked
  )
    return false;
  const first = split.children[index],
    second = split.children[index + 1];
  if (!first || !second) return false;
  return (
    first.settings?.resizable !== false &&
    second.settings?.resizable !== false &&
    !first.settings?.locked &&
    !second.settings?.locked &&
    !first.settings?.collapsed &&
    !second.settings?.collapsed &&
    (first.settings?.sizeMode ?? "flex") === "flex" &&
    (second.settings?.sizeMode ?? "flex") === "flex"
  );
}
...
function tabLabel(child: PaneNode): string {
  if (child.kind === "widget") {
    try {
      return registry.get(child.widgetId).title;
    } catch {
      return child.id;
    }
  }
...
function tabDragEnabled(tabPane: TabPane, child: PaneNode): boolean {
  return (
    !props.layoutLocked &&
    !tabPane.settings?.locked &&
    !child.settings?.locked &&
    props.paneDragEnabled(tabPane.id) &&
    props.paneDragEnabled(child.id)
  );
}
function activateTab(tabPane: TabPane, childId: string): void {
  emit("update:pane", setActiveTab(props.pane, tabPane.id, childId));
}
function onTabKeydown(
  ...
  if (event.key === "ArrowLeft")
    next = (index - 1 + tabPane.children.length) % tabPane.children.length;
...
function startResize(
  ...
  const startCoordinate =
    split.axis === "horizontal" ? event.clientX : event.clientY;
  ...
  const matches = (next: PointerEvent) =>
    pointerId === undefined ||
  ...
  const move = (next: PointerEvent) => {
    ...
    const coordinate =
      startSplit.axis === "horizontal" ? next.clientX : next.clientY;
    emit("update:pane", {
      ...startSplit,
      weights: resizePaneSplitWeights(
        startSplit,
        dividerIndex,
...
onMounted(() => {
  ...
  if (element)
    disposeSize = observeElementSize(element, (size) => {
...
<template>
  <div
    ...
    :class="[
      backgroundClass,
      {
        'wf-pane-host--edit': editMode,
        'wf-pane-host--split': pane.kind === 'split',
        'wf-pane-host--tabs': pane.kind === 'tabs',
        'wf-pane-host--stack': pane.kind === 'stack',
      },
    ]"
    :data-pane-id="pane.id"
    :data-pane-kind="pane.kind"
    :data-pane-size-mode="pane.settings?.sizeMode ?? 'flex'"
    :data-pane-collapsed="pane.settings?.collapsed || undefined"
    :data-pane-locked="pane.settings?.locked || undefined"
    :data-pane-host-type="hostType"
  ...
  >
    <template v-if="pane.kind === 'widget'"
      ><div class="wf-pane-host__widget-frame">
        <div
          v-if="actionChrome === 'pane' && visibleWidgetActions"
          class="wf-pane-host__actionbar"
        >
          <WidgetActionToolbar
            :bindings="widgetActions"
            :max-visible="actionMaxVisible"
            :compact="compactActions"
            :aria-label="`${pane.id} widget actions`"
          />
        </div>
        <WidgetHost
          ...
          :lifecycle="lifecycle"
          @actions-change="setWidgetActions"
        /></div></template
      ...
      ><template v-for="(child, index) in pane.children" :key="child.id"
        ><div class="wf-pane-host__cell" :style="childStyle(pane, index)">
          <PaneHost
            ...
            :edit-mode="editMode"
            :pane-drag-enabled="paneDragEnabled"
            :host-type="hostType"
            :active="active"
            :host-visible="hostVisible"
            :host-focused="hostFocused"
            action-chrome="pane"
        ...
        <div
          ...
          @pointerdown="
            startResize($event, pane, index)
          " /></template></template
    ><template v-else-if="pane.kind === 'tabs'"
      ><div
        class="wf-pane-host__tabbar"
        role="tablist"
        :aria-label="`${pane.id} tabs`"
        :data-tab-container-id="pane.id"
      >
        <button
          v-for="(child, index) in pane.children"
          :key="child.id"
          class="wf-pane-host__tab"
          :class="{ 'wf-pane-host__tab--active': child.id === pane.activeId }"
          role="tab"
          :aria-selected="child.id === pane.activeId"
          :tabindex="child.id === pane.activeId ? 0 : -1"
          :data-tab-pane-id="child.id"
          :data-tab-id="child.id"
          @click="activateTab(pane, child.id)"
          @keydown="onTabKeydown($event, pane, index)"
        >
          <span>{{ tabLabel(child) }}</span
          ><span
            v-if="!editMode"
            class="wf-pane-host__tab-drag-handle"
            data-tab-drag-handle
            :data-tab-drag-disabled="
              tabDragEnabled(pane, child) ? undefined : 'true'
            "
      ...
      <div class="wf-pane-host__tab-content">
        <div
          ...
          v-show="child.id === pane.activeId"
          :key="child.id"
          class="wf-pane-host__tab-panel"
          role="tabpanel"
          :data-tab-content-id="child.id"
          :aria-hidden="child.id !== pane.activeId"
        >
          <PaneHost
            :pane="child"
            :registry="registry"
            ...
            :pane-drag-enabled="paneDragEnabled"
            :host-type="hostType"
            :active="active && child.id === pane.activeId"
            :host-visible="hostVisible"
            :host-focused="hostFocused"
            action-chrome="pane"
            @update:pane="updateChild(child.id, $event)"
    ...
    ><template v-else
      ><div
        v-for="(child, index) in pane.children"
        :key="child.id"
        class="wf-pane-host__stack-layer"
        :style="stackChildStyle(index, child)"
        :data-stack-layer-id="child.id"
      >
        <PaneHost
          :pane="child"
          :registry="registry"
...
.wf-pane-host--background-transparent {
  background: transparent;
}
.wf-pane-host--background-canvas {
  background: var(--wf-color-canvas);
}
.wf-pane-host--background-surface {
...
.wf-pane-host[data-pane-edit-draggable="true"]:active {
  cursor: grabbing;
}
.wf-pane-host__widget-frame {
  display: flex;
  width: 100%;
  height: 100%;
  min-width: 0;
...
.wf-pane-host__actionbar {
  display: flex;
  flex: 0 0 auto;
  justify-content: flex-end;
  padding: 2px var(--wf-space-xs);
...
.wf-pane-host__divider--horizontal {
  cursor: ew-resize;
}
...
.wf-pane-host__divider--vertical {
  cursor: ns-resize;
}
...
.wf-pane-host__tab > span:first-child {
  min-width: 0;
  overflow: hidden;
  text-overflow: ellipsis;
}
.wf-pane-host__tab:hover {
  background: var(--wf-color-hover);
  color: var(--wf-color-text);
}
.wf-pane-host__tab:focus-visible {
  outline: 1px solid var(--wf-color-focus);
  outline-offset: -2px;
}
.wf-pane-host__tab--active {
  background: var(--wf-color-selected);
  color: var(--wf-color-text);
}
.wf-pane-host__tab--active .wf-pane-host__tab-drag-handle {
  color: var(--wf-color-text);
}
.wf-pane-host__tab-drag-handle {
  color: var(--wf-color-text-muted);
  font-size: var(--wf-font-size-xs);
  cursor: grab;
  touch-action: none;
}
.wf-pane-host__tab-drag-handle[data-tab-drag-disabled="true"] {
  cursor: default;
  opacity: 0.45;
}
.wf-pane-host__tab-content {
  position: relative;
  flex: 1 1 auto;
  min-width: 0;
  min-height: 0;
}
.wf-pane-host__tab-panel {
  position: absolute;
  inset: 0;
  min-width: 0;
  min-height: 0;
