...
import { surfaceStyleToCssVars, type LayoutSurfaceStyle } from '../core/layout-surface-style'
...
const surfaceStyleStyle = computed<Record<string, string>>(() => surfaceStyleToCssVars(props.surfaceStyle))
  ...
  <section class="wf-window-shell" :class="[`wf-window-shell--role-${windowRole}`,`wf-window-shell--chrome-${chrome}`,{'wf-window-shell--focused':visualFocused,'wf-window-shell--minimized':minimized,'wf-window-shell--maximized':maximized,'wf-window-shell--layout-locked':windowLocked,'wf-window-shell--headerless':!showHeader || windowLocked,'wf-window-shell--glass':glass}]" :data-window-instance-id="instanceId" :data-focused="focused ? 'true' : 'false'" :data-window-visual-focused="visualFocused ? 'true' : 'false'" :data-window-role="windowRole" :data-window-mode="minimized ? 'minimized' : maximized ? 'maximized' : 'normal'" :data-window-locked="windowLocked || undefined" :data-window-header="header" :data-window-chrome="chrome" :data-window-glass="glass || undefined" :role="ariaRole" :aria-modal="windowRole === 'modal' ? 'true' : undefined" :aria-label="resolvedTitle" :aria-expanded="minimized ? 'false' : 'true'" :tabindex="-1" :style="surfaceStyleStyle" @mouseenter="setHovered(true)" @mouseleave="setHovered(false)" @pointerdown="requestFocus">
    ...
    <header v-if="showHeader && !windowLocked" class="wf-window-shell__titlebar" :data-window-drag-handle="movable && !maximized ? '' : undefined">
      ...
      <div class="wf-window-shell__actions">
        ...
        <slot name="actions" />
        <button v-if="editMode && !windowLocked && !minimized" class="wf-window-shell__lock" type="button" data-window-lock :aria-label="lockLabel" :title="lockLabel" @pointerdown.stop @click.stop="requestLock"><span class="wf-window-shell__action-icon"><WfIcon :name="props.snapZone ? 'layout' : 'lock'" /></span><span class="wf-window-shell__lock-label">{{ props.snapZone ? 'Als Layout übernehmen' : 'Lock' }}</span></button>
        <button v-if="minimizable" class="wf-window-shell__minimize" type="button" :aria-label="minimized ? 'Restore window' : 'Minimize window'" @pointerdown.stop @click.stop="toggleMinimized"><span class="wf-window-shell__action-icon"><WfIcon :name="minimized ? 'restore' : 'minimize'" /></span></button>
        <button v-if="closable" class="wf-window-shell__close" type="button" aria-label="Close window" @pointerdown.stop @click.stop="requestClose"><span class="wf-window-shell__action-icon"><WfIcon name="close" /></span></button>
      </div>
    </header>
...
.wf-window-shell__lock{min-height:var(--wf-size-icon-button-size);padding:0 var(--wf-space-xs);display:inline-flex;align-items:center;gap:var(--wf-space-2xs);color:var(--wf-color-text-muted);background:transparent;border:0;border-radius:var(--wf-radius-sm);font:inherit;font-size:var(--wf-font-size-xs);cursor:pointer}.wf-window-shell__lock:hover{color:var(--wf-color-text);background:var(--wf-color-hover)}.wf-window-shell__lock:focus-visible{outline:2px solid var(--wf-color-focus);outline-offset:-2px}.wf-window-shell--layout-locked .wf-window-shell__content{padding:0}
.wf-window-shell{background:var(--wf-surface-background,var(--wf-window-shell-surface));border:0;border-top:var(--wf-surface-border-top-width,var(--wf-window-shell-border-width)) solid var(--wf-surface-border-top-color,var(--wf-window-shell-border));border-right:var(--wf-surface-border-right-width,var(--wf-window-shell-border-width)) solid var(--wf-surface-border-right-color,var(--wf-window-shell-border));border-bottom:var(--wf-surface-border-bottom-width,var(--wf-window-shell-border-width)) solid var(--wf-surface-border-bottom-color,var(--wf-window-shell-border));border-left:var(--wf-surface-border-left-width,var(--wf-window-shell-border-width)) solid var(--wf-surface-border-left-color,var(--wf-window-shell-border));border-radius:var(--wf-surface-radius,var(--wf-window-shell-radius));box-shadow:var(--wf-surface-shadow,var(--wf-window-shell-shadow))}.wf-window-shell__surface{border-radius:max(0px,calc(var(--wf-surface-radius,var(--wf-window-shell-radius)) - var(--wf-window-shell-border-width)));background:var(--wf-surface-background,var(--wf-window-shell-surface))}.wf-window-shell__titlebar{background:var(--wf-surface-background,var(--wf-window-shell-surface))}.wf-window-shell__content{padding:var(--wf-surface-padding-top,var(--wf-space-md)) var(--wf-surface-padding-right,var(--wf-space-md)) var(--wf-surface-padding-bottom,var(--wf-space-md)) var(--wf-surface-padding-left,var(--wf-space-md))}.wf-window-shell--layout-locked .wf-window-shell__content{padding:var(--wf-surface-padding-top,0) var(--wf-surface-padding-right,0) var(--wf-surface-padding-bottom,0) var(--wf-surface-padding-left,0)}
.wf-window-shell__lock .wf-icon, .wf-window-shell__minimize .wf-icon, .wf-window-shell__close .wf-icon { width: var(--wf-size-icon-size); height: var(--wf-size-icon-size); }
.wf-window-shell__header-action:focus-visible, .wf-window-shell__minimize:focus-visible, .wf-window-shell__close:focus-visible, .wf-window-shell__lock:focus-visible { outline-offset: 1px; }
@media (prefers-reduced-motion: reduce) { .wf-window-shell, .wf-window-shell * { transition: none !important; } }
</style>
