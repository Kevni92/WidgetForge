...
<template>
  <section class="wf-window-shell" :class="[`wf-window-shell--role-${windowRole}`,`wf-window-shell--chrome-${chrome}`,{'wf-window-shell--focused':focused,'wf-window-shell--minimized':minimized,'wf-window-shell--maximized':maximized,'wf-window-shell--headerless':!showHeader,'wf-window-shell--glass':glass}]" :data-window-instance-id="instanceId" :data-focused="focused ? 'true' : 'false'" :data-window-role="windowRole" :data-window-mode="minimized ? 'minimized' : maximized ? 'maximized' : 'normal'" :data-window-header="header" :data-window-chrome="chrome" :data-window-glass="glass || undefined" :role="ariaRole" :aria-modal="windowRole === 'modal' ? 'true' : undefined" :aria-label="resolvedTitle" :aria-expanded="minimized ? 'false' : 'true'" :tabindex="windowRole === 'modal' ? -1 : undefined" @mouseenter="setHovered(true)" @mouseleave="setHovered(false)" @pointerdown="requestFocus">
    <header v-if="showHeader" class="wf-window-shell__titlebar" :data-window-drag-handle="movable && !maximized ? '' : undefined">
      ...
      <div class="wf-window-shell__actions">
        ...
        <div v-if="maximizable && !minimized" class="wf-window-shell__layout-action" @mouseenter="openLayoutPicker">
          <button class="wf-window-shell__maximize" type="button" :aria-label="maximized ? 'Restore window' : 'Window layouts'" :aria-expanded="layoutPickerOpen" @pointerdown.stop @focus="openLayoutPicker" @click.stop="toggleLayoutPicker"><span class="wf-window-shell__action-icon" aria-hidden="true">{{ maximized ? '❐' : '□' }}</span></button>
...
<style scoped>
.wf-window-shell{display:flex;min-width:0;min-height:0;flex-direction:column;overflow:visible;color:var(--wf-color-text);background:var(--wf-color-surface-window);border:1px solid var(--wf-color-border-strong);border-radius:var(--wf-radius-md);box-shadow:var(--wf-shadow-md);font-family:var(--wf-font-family)}.wf-window-shell--focused{border-color:var(--wf-color-focus)}.wf-window-shell--maximized{border-radius:0}.wf-window-shell--role-utility{background:var(--wf-color-surface-floating);border-color:var(--wf-color-border-floating);box-shadow:var(--wf-shadow-lg)}.wf-window-shell--role-overlay{background:var(--wf-color-surface-overlay);border-color:var(--wf-color-border-overlay);box-shadow:var(--wf-shadow-lg)}.wf-window-shell--role-modal{background:var(--wf-color-surface-modal);border-color:var(--wf-color-border-modal);box-shadow:var(--wf-shadow-lg)}.wf-window-shell--chrome-borderless{border-color:transparent}.wf-window-shell--chrome-none{border-color:transparent;border-radius:0;box-shadow:none}.wf-window-shell--glass{background:color-mix(in srgb,var(--wf-color-surface-window) 78%,transparent);backdrop-filter:blur(10px)}.wf-window-shell__titlebar{display:flex;min-height:var(--wf-size-titlebar-height);align-items:center;justify-content:space-between;gap:var(--wf-space-sm);padding:0 var(--wf-space-sm) 0 var(--wf-space-md);background:var(--wf-color-surface-window);border-bottom:1px solid var(--wf-color-border-subtle);user-select:none}.wf-window-shell--role-utility .wf-window-shell__titlebar{background:var(--wf-color-surface-floating);border-bottom-color:var(--wf-color-border-floating)}.wf-window-shell--role-overlay .wf-window-shell__titlebar{background:var(--wf-color-surface-overlay);border-bottom-color:var(--wf-color-border-overlay)}.wf-window-shell--role-modal .wf-window-shell__titlebar{background:var(--wf-color-surface-modal);border-bottom-color:var(--wf-color-border-modal)}.wf-window-shell--glass .wf-window-shell__titlebar{background:color-mix(in srgb,var(--wf-color-surface-window) 78%,transparent)}.wf-window-shell--chrome-borderless .wf-window-shell__titlebar,.wf-window-shell--chrome-none .wf-window-shell__titlebar{border-bottom-color:transparent}.wf-window-shell--minimized .wf-window-shell__titlebar{border-bottom:0}.wf-window-shell__leading,.wf-window-shell__actions{display:flex;min-width:0;align-items:center;gap:var(--wf-space-xs)}.wf-window-shell__leading{flex:1}.wf-window-shell__title{min-width:0;overflow:hidden;font-size:var(--wf-font-size-sm);font-weight:var(--wf-font-weight-medium);text-overflow:ellipsis;white-space:nowrap}.wf-window-shell__icon{flex:0 0 auto;color:var(--wf-color-accent)}.wf-window-shell__badge{flex:0 0 auto;padding:1px var(--wf-space-xs);border:1px solid var(--wf-color-border-subtle);border-radius:var(--wf-radius-sm);color:var(--wf-color-accent);font-size:var(--wf-font-size-xs)}.wf-window-shell__status{color:var(--wf-color-text-muted);font-size:var(--wf-font-size-xs);white-space:nowrap}.wf-window-shell__actions{position:relative}.wf-window-shell__layout-action{position:relative}.wf-window-shell__header-action,.wf-window-shell__minimize,.wf-window-shell__maximize,.wf-window-shell__close{min-width:var(--wf-size-control-height);height:var(--wf-size-control-height);padding:0 var(--wf-space-xs);color:var(--wf-color-text-muted);background:transparent;border:0;border-radius:var(--wf-radius-sm);font:inherit;cursor:pointer}.wf-window-shell__header-action:hover:not(:disabled),.wf-window-shell__minimize:hover,.wf-window-shell__maximize:hover,.wf-window-shell__close:hover{color:var(--wf-color-text);background:var(--wf-color-hover)}.wf-window-shell__header-action:focus-visible,.wf-window-shell__minimize:focus-visible,.wf-window-shell__maximize:focus-visible,.wf-window-shell__close:focus-visible{outline:2px solid var(--wf-color-focus);outline-offset:-2px}.wf-window-shell__header-action:disabled{cursor:default;opacity:.5}.wf-window-shell__content{min-width:0;min-height:0;flex:1;padding:var(--wf-space-md);overflow:hidden}.wf-window-shell__content :deep(.wf-pane-host){width:100%;height:100%}
.wf-window-shell__header-action,.wf-window-shell__minimize,.wf-window-shell__maximize,.wf-window-shell__close{min-width:var(--wf-size-icon-button-size);height:var(--wf-size-icon-button-size)}.wf-window-shell__action-icon{display:inline-grid;width:var(--wf-size-icon-size);height:var(--wf-size-icon-size);place-items:center;line-height:1;font-size:var(--wf-size-icon-size)}
</style>
