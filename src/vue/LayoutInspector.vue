...
import type { WfIconName } from './WfIcon.vue'
...
function borderIcon(side: LayoutSurfaceSide): WfIconName {
  return `border-${side}` as WfIconName
}
  ...
  <aside ref="root" class="wf-layout-inspector" :class="{ 'wf-layout-inspector--collapsed': inspectorMode === 'minimized', 'wf-layout-inspector--floating': inspectorMode === 'floating', 'wf-layout-inspector--minimized': inspectorMode === 'minimized' }" :style="inspectorStyle" :data-layout-inspector-mode="inspectorMode" data-workspace-selection-actions data-layout-inspector aria-label="Layout Inspector" @focusout="onFocusOut" @blur.capture="commitOnBlur" @keydown="onKeydown">
    <template v-if="inspectorMode === 'minimized'">
      <div class="wf-layout-inspector__minimized" data-layout-inspector-minimized>
        <button ref="restoreControl" type="button" data-layout-inspector-toggle data-layout-inspector-restore aria-label="Restore layout inspector" @click="toggleMinimized"><WfIcon name="inspector" /><span class="wf-layout-inspector__minimized-label">Inspector</span></button>
    ...
    <template v-else>
      ...
      <div class="wf-layout-inspector__actions">
        <button class="wf-workspace-selection-actions__layout" type="button" data-window-selection-layout :aria-label="`Advanced layout for ${currentWindow.title}`" @click="emit('layout', currentWindow.instanceId)"><WfIcon name="layout" /><span>Advanced layout…</span></button>
        <button class="wf-workspace-selection-actions__toggle" type="button" data-window-selection-lock :aria-pressed="currentWindow.layoutLocked ? 'true' : 'false'" :aria-label="`${currentWindow.layoutLocked ? 'Unlock' : 'Lock'} window ${currentWindow.title}`" :title="`${currentWindow.layoutLocked ? 'Unlock' : 'Lock'} window ${currentWindow.title}`" @click="currentWindow.layoutLocked ? emit('unlock', currentWindow.instanceId) : emit('lock', currentWindow.instanceId)"><WfIcon :name="currentWindow.layoutLocked ? 'unlock' : 'lock'" /><span>{{ currentWindow.layoutLocked ? 'Unlock' : 'Lock' }}</span></button>
      </div>
      ...
      <template v-if="styleTab === 'styles' && (currentSelection || currentWindow)">
        <div id="layout-inspector-panel-styles" role="tabpanel" aria-labelledby="layout-inspector-tab-styles" data-layout-inspector-styles>
          ...
          <button type="button" data-style-reset @click="resetStyles"><WfIcon name="reset" /><span>Reset all styles</span></button>
        </div>
      </template>
...
<style scoped>
.wf-layout-inspector { position: absolute; top: calc(var(--wf-space-sm) + var(--wf-size-control-height) + var(--wf-space-xs)); right: var(--wf-space-sm); bottom: var(--wf-space-sm); z-index: calc(var(--wf-layer-overlay) + 3); box-sizing: border-box; display: grid; align-content: start; width: min(360px, calc(100% - var(--wf-space-md))); max-width: calc(100% - var(--wf-space-md)); gap: var(--wf-space-sm); padding: var(--wf-space-md); overflow: auto; border: 1px solid var(--wf-color-border-floating); border-radius: var(--wf-radius-md); background: var(--wf-editor-panel-background); box-shadow: var(--wf-shadow-md); color: var(--wf-color-text); }
...
.wf-layout-inspector__header-actions button { display: inline-flex; align-items: center; justify-content: center; gap: var(--wf-space-2xs); min-width: var(--wf-size-icon-button-size); min-height: var(--wf-size-icon-button-size); padding-inline: var(--wf-space-xs); }
.wf-layout-inspector__button-label { white-space: nowrap; }
.wf-layout-inspector__actions { flex-wrap: wrap; }
.wf-layout-inspector__actions button, .wf-layout-inspector__minimized button, [data-style-padding-linked], [data-style-reset] { display: inline-flex; align-items: center; justify-content: center; gap: var(--wf-space-2xs); }
.wf-layout-inspector fieldset { padding: var(--wf-space-sm) 0 0; border: 0; border-top: 1px solid var(--wf-color-border); border-radius: 0; }
.wf-layout-inspector legend { padding: 0; font-weight: var(--wf-font-weight-bold); letter-spacing: .04em; text-transform: uppercase; }
.wf-layout-inspector input, .wf-layout-inspector select, .wf-layout-inspector button { box-sizing: border-box; }
.wf-layout-inspector input, .wf-layout-inspector select { min-height: var(--wf-size-control-height-compact); }
.wf-layout-inspector output { display: inline-flex; min-height: var(--wf-size-control-height-compact); align-items: center; justify-content: flex-end; padding-inline: var(--wf-space-xs); border: 1px solid var(--wf-color-border-subtle); border-radius: var(--wf-radius-sm); background: var(--wf-color-surface); text-align: right; }
.wf-layout-inspector__constraint { position: relative; padding-right: calc(var(--wf-space-xs) + var(--wf-size-icon-button-size)); }
.wf-layout-inspector__constraint header { padding-right: var(--wf-space-xs); }
.wf-layout-inspector__constraint .wf-layout-inspector__disconnect { position: absolute; top: var(--wf-space-xs); right: var(--wf-space-xs); display: grid; width: var(--wf-size-icon-button-size); min-width: var(--wf-size-icon-button-size); height: var(--wf-size-icon-button-size); min-height: var(--wf-size-icon-button-size); place-items: center; padding: 0; }
.wf-layout-inspector__tabs { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 2px; padding: 2px; border: 1px solid var(--wf-color-border); border-radius: var(--wf-radius-sm); background: var(--wf-color-surface); }
.wf-layout-inspector__tab { display: inline-flex; align-items: center; justify-content: center; gap: var(--wf-space-2xs); min-height: var(--wf-size-control-height-compact); border: 0 !important; border-radius: var(--wf-radius-sm) !important; background: transparent !important; }
.wf-layout-inspector__tab--active { border: 1px solid var(--wf-editor-selection-color) !important; background: var(--wf-color-selected) !important; color: var(--wf-editor-selection-color) !important; }
.wf-layout-inspector__side-control--enabled { border-color: var(--wf-editor-selection-color) !important; background: var(--wf-color-selected) !important; color: var(--wf-editor-selection-color) !important; }
[data-style-padding-linked][aria-pressed="true"] { border-color: var(--wf-editor-selection-color); color: var(--wf-editor-selection-color); }
[data-style-reset] { justify-self: start; }
@media (prefers-reduced-motion: reduce) { .wf-layout-inspector, .wf-layout-inspector * { scroll-behavior: auto; transition: none !important; } }
