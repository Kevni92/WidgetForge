...
function frameStyle(): Record<string, string> {
  ...
  return { left: `${geometry.position.x}px`, top: `${geometry.position.y}px`, width: `${geometry.size.width}px`, height: mode === 'minimized' ? 'var(--wf-size-titlebar-height)' : `${geometry.size.height}px`, zIndex: `calc(var(--wf-layer-window) + ${zIndex})`, opacity: String(options.opacity) }
}
function finishInteraction(): void { disposeInteraction?.() }
function startInteraction(event: PointerEvent): void {
  if (props.layoutLocked || props.window.mode === 'minimized') return
  ...
  const session: InteractionSession = { pointerId: typeof event.pointerId === 'number' ? event.pointerId : undefined, captureTarget: target, startX: event.clientX, startY: event.clientY, startGeometry: { position: { ...props.window.geometry.position }, size: { ...props.window.geometry.size } }, handle: handle ?? null, unsnapPending: !handle && props.window.snap !== null, snapZone: null, windowDrop: null }
  interactionKind.value = session.handle ? 'resize' : 'move'
  if (session.pointerId !== undefined && typeof session.captureTarget.setPointerCapture === 'function') { try { session.captureTarget.setPointerCapture(session.pointerId) } catch { /* optional */ } }
  ...
  const updatePreviews = (pointerEvent: PointerEvent): void => {
    ...
    if (session.windowDrop) { session.snapZone = null; props.previewSnap?.(props.window.instanceId, null); return }
  ...
  const onPointerMove = (pointerEvent: PointerEvent): void => {
    ...
    if (!session.handle && session.unsnapPending) {
      ...
      if (unsnapped) { session.startGeometry = { position: { ...unsnapped.geometry.position }, size: { ...unsnapped.geometry.size } }; session.startX = pointerEvent.clientX; session.startY = pointerEvent.clientY }
      session.unsnapPending = false
    }
    const delta = { x: pointerEvent.clientX - session.startX, y: pointerEvent.clientY - session.startY }
    const geometry = session.handle ? resizeWindow(session.startGeometry, session.handle, delta, props.window.constraints, effectiveContainerSize()) : moveWindow(session.startGeometry, delta, effectiveContainerSize())
    manager.setGeometry(props.window.instanceId, geometry, 'user'); updatePreviews(pointerEvent)
  }
  const cleanup = (): void => {
    ...
    if (!session.handle) { props.previewSnap?.(props.window.instanceId, null); props.previewWindowDrop?.(props.window.instanceId, null) }
    if (session.pointerId !== undefined && typeof session.captureTarget.releasePointerCapture === 'function') { try { session.captureTarget.releasePointerCapture(session.pointerId) } catch { /* optional */ } }
    if (disposeInteraction === cleanup) disposeInteraction = null
    interactionKind.value = null
  }
  const onPointerEnd = (pointerEvent: PointerEvent): void => { if (!pointerMatches(pointerEvent)) return; if (pointerEvent.type === 'pointerup' && !session.handle) { if (session.windowDrop) props.commitWindowDrop?.(props.window.instanceId, session.windowDrop); else if (session.snapZone) props.commitSnap?.(props.window.instanceId, session.snapZone) } cleanup() }
  disposeInteraction = cleanup
  globalThis.window.addEventListener('pointermove', onPointerMove); globalThis.window.addEventListener('pointerup', onPointerEnd); globalThis.window.addEventListener('pointercancel', onPointerEnd)
}
...
function updateRootPane(pane: PaneNode): void { if (!props.layoutLocked) manager.setRootPane(props.window.instanceId, pane, 'user') }
onBeforeUnmount(finishInteraction)
</script>

<template>
  <div class="wf-window-frame" :class="{ 'wf-window-frame--interacting': interactionKind, 'wf-window-frame--minimized': window.mode === 'minimized', 'wf-window-frame--layout-locked': layoutLocked }" :data-window-instance-id="window.instanceId" :data-window-z-index="window.zIndex" :data-window-layer="window.options.layer" :data-window-mode="window.mode" :data-window-interaction="interactionKind ?? 'none'" :data-layout-locked="layoutLocked || undefined" :style="frameStyle()" @pointerdown.capture="startInteraction">
    <WindowShell :registry="registry" :pane="window.rootPane" :instance-id="window.instanceId" :title="window.title" :focused="window.focused" :minimized="window.mode === 'minimized'" :closable="window.options.closable" :minimizable="window.options.minimizable" :movable="window.options.movable" :header="window.options.header" :lifecycle="lifecycleController" @focus="focusWindow" @close="closeWindow" @minimize="minimizeWindow" @restore="restoreWindow" @update:pane="updateRootPane" />
    <template v-if="window.mode === 'normal' && window.options.resizable && !layoutLocked"><div v-for="handle in resizeHandles" :key="handle" class="wf-window-frame__resize-handle" :class="`wf-window-frame__resize-handle--${handle}`" :data-window-resize-handle="handle" aria-hidden="true" /></template>
  </div>
</template>

<style scoped>
.wf-window-frame{position:absolute;min-width:0;min-height:0;touch-action:none}.wf-window-frame :deep(.wf-window-shell){width:100%;height:100%}.wf-window-frame :deep([data-window-drag-handle]){cursor:move}.wf-window-frame--minimized :deep([data-window-drag-handle]),.wf-window-frame--layout-locked :deep([data-window-drag-handle]){cursor:default}.wf-window-frame__resize-handle{position:absolute;z-index:2;touch-action:none}.wf-window-frame__resize-handle--top,.wf-window-frame__resize-handle--bottom{right:8px;left:8px;height:8px;cursor:ns-resize}.wf-window-frame__resize-handle--top{top:-4px}.wf-window-frame__resize-handle--bottom{bottom:-4px}.wf-window-frame__resize-handle--left,.wf-window-frame__resize-handle--right{top:8px;bottom:8px;width:8px;cursor:ew-resize}.wf-window-frame__resize-handle--left{left:-4px}.wf-window-frame__resize-handle--right{right:-4px}.wf-window-frame__resize-handle--top-left,.wf-window-frame__resize-handle--top-right,.wf-window-frame__resize-handle--bottom-left,.wf-window-frame__resize-handle--bottom-right{width:12px;height:12px}.wf-window-frame__resize-handle--top-left{top:-5px;left:-5px;cursor:nwse-resize}.wf-window-frame__resize-handle--top-right{top:-5px;right:-5px;cursor:nesw-resize}.wf-window-frame__resize-handle--bottom-left{bottom:-5px;left:-5px;cursor:nesw-resize}.wf-window-frame__resize-handle--bottom-right{right:-5px;bottom:-5px;cursor:nwse-resize}
</style>
