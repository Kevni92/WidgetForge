...
Während der Layout-Dialog geöffnet ist, trennt er den persistierten `Current`-Wert von einer nicht persistierenden `Preview`. Jeder gültige Draft wird mit demselben Resolver wie beim Speichern aufgelöst und als `WindowLayoutDialogPreview` an den Workspace-Host gegeben. Der Host rendert daraus eine dekorative Ghost-Fläche und verwendet den Draft-Spec für die Beziehungslinien. Ein ungültiger Draft blendet die Vorschau aus und deaktiviert `Save`; weder Preview noch `Cancel` verändern `WindowManager`, History oder Persistenz. `Save layout` bleibt der einzige semantische Commit.
