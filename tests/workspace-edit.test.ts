...
describe('workspace edit controller', () => {
  it('separates explicit edit, temporary ctrl edit and locked mode', () => {
    ...
    edit.setTemporaryEdit(true); expect(edit.state.editActive).toBe(true)
    edit.selectPane(selection); expect(edit.state.selection).toEqual(selection)
    edit.setTemporaryEdit(false); expect(edit.state.selection).toBeNull()
    edit.setMode('edit'); edit.selectPane(selection); expect(edit.state.selection).toEqual(selection)
    edit.setMode('locked'); expect(edit.state).toMatchObject({ mode: 'locked', editActive: false, locked: true, selection: null })
    edit.setTemporaryEdit(true); expect(edit.state.editActive).toBe(false)
  })

  it('serializes and restores workspace and pane lock state', () => {
    const edit = createWorkspaceEditController({ mode: 'edit' })
    edit.selectPane(selection); edit.setPaneLocked(selection, true)
...
describe('generic pane edit actions', () => {
  const root = createSplitPane({ id: 'root', axis: 'horizontal', children: [createWidgetPane({ id: 'left', widgetId: 'demo.a', instanceId: 'stable' }), createWidgetPane({ id: 'right', widgetId: 'demo.b' })] })
  ...
  it('retargets widgets without changing pane or widget instance identity and removes nested panes', () => {
    const retargeted = retargetWidgetPane(root, 'left', 'demo.c', { compact: true }); const pane = findPane(retargeted, 'left')
    ...
    expect(removePaneForEdit(root, 'left')?.id).toBe('right')
  })
})
