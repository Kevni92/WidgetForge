...
import { DataTable, EmptyState, ErrorState, LoadingState, useData, useLinkedSelection, useNotifications, type DataTableColumn } from 'widgetforge'
...
const state=useData(economyKey),linked=useLinkedSelection<string,ViewState>(colonySelectionKey,{read:(value)=>value.selection,write:(value,selection)=>({...value,selection})}),notifications=useNotifications()
...
function submitDemoOrder():void{const name=location.value?.name??'current location';notifications.push({title:'Order staged',message:`A deterministic demo order was staged for ${name}.`,tone:'success'})}
</script>
